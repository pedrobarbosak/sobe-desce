...
/**
 * The room before, between and around sittings: who is here, whether there are enough of
 * them, and the way to the table. `embedded` is the drawer over the felt, where the
 * table is already live and the room is one column.
 */
export function LobbyView({ gameId, embedded = false }: { gameId: Id<"games">; embedded?: boolean }) {
  ...
  const short = seatedCount < MIN_SEATS;
  ...
  const rosterOpen = !finished && (isCampaign || !sittingActive);
  ...
  const canArrange = isOwner && !finished;
  ...
  const canOpenSitting = isCampaign && !sittingActive && !finished && me !== null && canStart && !ownerOnline;
  // The campaign's one question for everybody: are you playing tonight?
  const askToPlay = isCampaign && me !== null && !sittingActive && !finished && !seatedNow.has(me._id);
  ...
  return (
    <div className={embedded ? "grid gap-4" : "grid gap-4 lg:grid-cols-[1fr_20rem] lg:grid-rows-[auto_1fr]"}>
      ...
      <div className={cols.main}>
        ...
        {showReady && (
          <Panel className={canStart ? "border-gold-400/60 bg-gold-400/10" : ""} role="status">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0 flex-1">
                <p className="font-display text-xl font-bold text-cream-50">{canStart ? t("lobby.readyTitle") : t("lobby.waitingTitle")}</p>
                <p className="mt-0.5 text-sm text-cream-100/80">
                  {short
                    ? t("lobby.needMore", { count: MIN_SEATS - seatedCount, seated: seatedCount })
                    : tooMany
                      ? t("lobby.tooMany", { max: game.config.seats })
                      : t("lobby.sittingPreview", { count: seatedCount, cap: cap ?? 0 })}
                </p>
                {short && <p className="mt-0.5 text-xs text-cream-100/60">{isCampaign ? t("lobby.needCheckIns") : t("lobby.inviteOrBots")}</p>}
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                {!isOwner && canOpenSitting && (
                  <Button disabled={busy} onClick={goStart}>
                    {t("lobby.startAnyone")}
                  </Button>
                )}
              </div>
        ...
        {finished ? (
          ...
          <Panel>
            <h2 className="mb-3 font-display text-xl font-bold text-cream-50">
                  ...
                  <li key={p._id} className="flex flex-wrap items-center gap-3 py-2.5">
                    ...
                    {scoring?.playerId === p._id && (
                      ...
                      >
                        ...
                        <div className="mt-2 flex gap-2">
                          ...
                          <Button type="button" variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => setScoring(null)}>
                            {t("party.cancel")}
                          </Button>
                        </div>
                      </form>
                    ...
                    {linking === p._id && (
                      <div className="mt-2 w-full border-t border-white/10 pt-2">
                        ...
                        <div className="mt-2">
                          <Button type="button" variant="ghost" className="px-3 py-1.5 text-xs" onClick={() => setLinking(null)}>
                            {t("party.cancel")}
                          </Button>
                        </div>
                      </div>
