...
import { POWERUP_ICONS, type PartyView } from "./party";
...
export function RoundResult({
  ...
  return (
    <motion.div
      ...
      className={`mx-auto w-full rounded-2xl border border-gold-400/50 bg-black/75 p-5 text-center shadow-2xl short:p-3 ${gameOver ? "max-w-md short:max-w-[19rem] short:shrink-0" : "max-w-md short:max-w-2xl"}`}
    >
      ...
      {party && (
        <div className="mt-2 flex justify-center">
          <PartyRecap party={party} seats={seats} />
        </div>
      )}
      ...
      {!gameOver && (
        <ul className="mt-3 divide-y divide-white/10 short:grid short:grid-cols-2 short:gap-x-4 short:divide-y-0">
          {ordered.map((s) => (
            <li key={s.seat} className="flex items-center gap-3 py-2 text-sm short:gap-2 short:py-1">
