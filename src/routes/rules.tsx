...
const PT = {
  sections: [
    ...
    { h: "Festa", p: ["Na variante Festa, cada ronda vira uma reviravolta à vista de todos antes de se escolher o trunfo. Tudo o resto joga-se como no clássico.", "Desce: ganha a carta mais baixa, quem puder ir por baixo tem de ir, e sai-se com o trunfo mais baixo em vez do ás. Naipe de ouro: um naipe sorteado (nunca copas) conta a dobrar se for o trunfo. Última vaza ×3: a quinta vaza vale por três. Zero paga: quem não fizer vazas tira a penalização em vez de a somar, e toda a gente joga para perder. Sem trunfo: salta-se a escolha do trunfo e só o naipe de saída pode ganhar. Cartas à vista: todas as mãos ficam à vista assim que o trunfo está decidido. Passa à esquerda: depois das trocas, quem está em jogo passa uma carta ao jogador da esquerda. Todos jogam: ninguém pode passar. Relâmpago: jogadas de 8 segundos. Como calhou: sem trocas e ninguém passa."] },
...
const EN = {
  sections: [
    ...
    { h: "Party", p: ["In the party variant every round flips a twist for all to see before the trump is chosen. Everything else plays as classic.", "Desce: the lowest card wins, you must go under if you can, and the lowest trump leads instead of the Ace. Golden suit: a drawn suit (never hearts) doubles the round if it becomes trump. Last trick \u00d73: the fifth trick is worth three. Blank pays: winning no tricks pays the penalty out instead of in, so everybody plays to lose. No trump: the trump phase is skipped and only the led suit can win. Open hands: every hand is face up once the trump is settled. Pass left: after the discards, everyone still in passes one card to the player on their left. All in: nobody may sit out. Lightning: 8-second turns. As dealt: no discards and nobody sits out."] },
